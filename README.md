...
- create_dataset: Creates a .csv based on multiple .json files extracted via the Codeforces API with various submissions from different users.
- preprocess_dataset: Preprocesses the generated .csv. Also splits the dataset into train, validation and test folds.
- predict: Using the test dataset, run various predicts with different methods. All the methods should use the same model.
...
There is a `parameters.yaml` file that allows you to specify various parameters for each task. You should change the paths such that it matches your use case. The parameters you should probably tweak are:
- path_to_test_data - Path to Codeforces-R1 generated tests
- path_to_contest_data - Path to Codeforces-R1 contest data
